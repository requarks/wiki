  ...
  describe('buildPageTree - child_position functionality', () => {
    it('should assign 0-based positions starting from 0', () => {
      const pages = [
        ...
        { id: 2, path: 'page2', localeCode: 'en', title: 'Page 2', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 3, path: 'page3', localeCode: 'en', title: 'Page 3', isPrivate: false, privateNS: null, siteId: 'site-1' }
      ]
      const tree = buildPageTree(pages, 'default-site')
      // All are root-level pages, should have sequential positions
      expect(tree.find(n => n.pageId === 1).child_position).toBe(0)
      expect(tree.find(n => n.pageId === 2).child_position).toBe(1)
      expect(tree.find(n => n.pageId === 3).child_position).toBe(2)
    })

    it('should assign positions with folders first, then pages alphabetically', () => {
      const pages = [
        { id: 1, path: 'zebra', localeCode: 'en', title: 'zebra', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 2, path: 'apple/child', localeCode: 'en', title: 'child', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 3, path: 'banana', localeCode: 'en', title: 'banana', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 4, path: 'zoo/item', localeCode: 'en', title: 'item', isPrivate: false, privateNS: null, siteId: 'site-1' }
      ]
      const tree = buildPageTree(pages, 'default-site')
      // Root level should have folders first (apple, zoo), then pages (banana, zebra)
      const rootNodes = tree.filter(n => n.parent === null)
      const appleNode = rootNodes.find(n => n.title === 'apple' && n.isFolder)
      const zooNode = rootNodes.find(n => n.title === 'zoo' && n.isFolder)
      const bananaNode = rootNodes.find(n => n.title === 'banana' && !n.isFolder)
      const zebraNode = rootNodes.find(n => n.title === 'zebra' && !n.isFolder)
      expect(appleNode.child_position).toBe(0)
      expect(zooNode.child_position).toBe(1)
      expect(bananaNode.child_position).toBe(2)
      expect(zebraNode.child_position).toBe(3)
    })

    it('should maintain independent position sequences per parent', () => {
      const pages = [
        { id: 1, path: 'folder1/child1', localeCode: 'en', title: 'child1', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 2, path: 'folder1/child2', localeCode: 'en', title: 'child2', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 3, path: 'page1', localeCode: 'en', title: 'page1', isPrivate: false, privateNS: null, siteId: 'site-1' }
      ]
      const tree = buildPageTree(pages, 'default-site')
      // Root level
      const folder1Node = tree.find(n => n.title === 'folder1' && n.isFolder)
      const page1Node = tree.find(n => n.pageId === 3)
      expect(folder1Node.child_position).toBe(0) // folder first
      expect(page1Node.child_position).toBe(1)   // then page
      // Under folder1 (positions should restart from 0)
      const child1Node = tree.find(n => n.pageId === 1)
      const child2Node = tree.find(n => n.pageId === 2)
      expect(child1Node.child_position).toBe(0)
      expect(child2Node.child_position).toBe(1)
    })

    it('should handle nested folder structure correctly', () => {
      const pages = [
        { id: 1, path: 'earth/tree/leaves/catalog/item1', localeCode: 'en', title: 'item1', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 2, path: 'earth/tree/leaves/green-leaf', localeCode: 'en', title: 'green-leaf', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 3, path: 'earth/tree/leaves/red-leaf', localeCode: 'en', title: 'red-leaf', isPrivate: false, privateNS: null, siteId: 'site-1' }
      ]
      const tree = buildPageTree(pages, 'default-site')
      // Find the nodes under earth/tree/leaves
      const catalogNode = tree.find(n => n.path === 'earth/tree/leaves/catalog' && n.isFolder)
      const greenLeafNode = tree.find(n => n.pageId === 2)
      const redLeafNode = tree.find(n => n.pageId === 3)
    ...
    it('should handle case-sensitive alphabetical sorting', () => {
      const pages = [
        { id: 1, path: 'zebra', localeCode: 'en', title: 'zebra', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 2, path: 'Apple', localeCode: 'en', title: 'Apple', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 3, path: 'banana', localeCode: 'en', title: 'banana', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 4, path: 'Zebra', localeCode: 'en', title: 'Zebra', isPrivate: false, privateNS: null, siteId: 'site-1' }
      ]
      const tree = buildPageTree(pages, 'default-site')
      // Case-sensitive: uppercase comes before lowercase
      // Expected order: Apple, Zebra, banana, zebra
      expect(tree.find(n => n.title === 'Apple').child_position).toBe(0)
      expect(tree.find(n => n.title === 'Zebra').child_position).toBe(1)
      expect(tree.find(n => n.title === 'banana').child_position).toBe(2)
      expect(tree.find(n => n.title === 'zebra').child_position).toBe(3)
    })

    it('should handle multiple sites independently', () => {
      const pages = [
        { id: 1, path: 'page1', localeCode: 'en', title: 'page1', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 2, path: 'page2', localeCode: 'en', title: 'page2', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 3, path: 'page1', localeCode: 'en', title: 'page1', isPrivate: false, privateNS: null, siteId: 'site-2' },
        { id: 4, path: 'page2', localeCode: 'en', title: 'page2', isPrivate: false, privateNS: null, siteId: 'site-2' }
      ]
      const tree = buildPageTree(pages, 'default-site')
      // Each site should have independent 0-based sequences
      const site1Pages = tree.filter(n => n.siteId === 'site-1' && n.pageId)
      const site2Pages = tree.filter(n => n.siteId === 'site-2' && n.pageId)
      expect(site1Pages[0].child_position).toBe(0)
      expect(site1Pages[1].child_position).toBe(1)
      expect(site2Pages[0].child_position).toBe(0)
      expect(site2Pages[1].child_position).toBe(1)
    })
    ...
    it('should handle empty parent (root level) correctly', () => {
      const pages = [
        { id: 1, path: 'folder/page', localeCode: 'en', title: 'page', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 2, path: 'page', localeCode: 'en', title: 'page', isPrivate: false, privateNS: null, siteId: 'site-1' }
      ]
      const tree = buildPageTree(pages, 'default-site')
      const rootNodes = tree.filter(n => n.parent === null)
    ...
    it('should handle complex multi-level structure', () => {
      const pages = [
        { id: 1, path: 'readme', localeCode: 'en', title: 'readme', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 2, path: 'license', localeCode: 'en', title: 'license', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 3, path: 'docs/intro', localeCode: 'en', title: 'intro', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 4, path: 'docs/api/endpoints', localeCode: 'en', title: 'endpoints', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 5, path: 'docs/api/auth', localeCode: 'en', title: 'auth', isPrivate: false, privateNS: null, siteId: 'site-1' },
        { id: 6, path: 'assets/logo', localeCode: 'en', title: 'logo', isPrivate: false, privateNS: null, siteId: 'site-1' }
      ]
      const tree = buildPageTree(pages, 'default-site')
