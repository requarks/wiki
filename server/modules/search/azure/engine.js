  ...
  async query(q, opts) {
    try {
      ...
      if (results.result.value.length < 5) {
        ...
        try {
          const suggestResp = await fetch(`https://${this.config.serviceName}.search.windows.net/indexes/${this.config.indexName}/docs/autocomplete?api-version=2017-11-11-Preview`, {
            method: 'POST',
            headers: {
          ...
          if (!suggestResp.ok) {
            throw new Error(`Unexpected response code ${suggestResp.status}`)
          }
