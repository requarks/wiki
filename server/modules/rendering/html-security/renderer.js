...
module.exports = {
  async init(input, config) {
    if (config.safeHTML) {
      ...
      console.log ("Before plantuml processing")
      ...
      DOMPurify.addHook('uponSanitizeElement',  (node, data) => {
        ...
        if (data.tagName === 'object') {
          ...
          if (!( 'data' in node.attributes
            ...
            console.log ("Mismatched attributes - removing")
          ...
          if (dataAttribute.startsWith(`${pumlServer}/${pumlImageFormat}`)) {
            console.log ("Plantuml node found - forcing atribute values")
            isPumlNode=true

            //forcing allowed attribute values
            node.setAttribute ('data', dataAttribute )
