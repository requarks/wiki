...
module.exports = {
  ...
  async init() {
    ...
    await this.git.raw([
      ...
      "--bool",
      "http.sslVerify",
      _.toString(this.config.verifySSL)
    ]);
    switch (this.config.authType) {
      ...
      default:
        ...
        if (_.startsWith(this.config.repoUrl, "http")) {
          ...
          originUrl = `https://${this.config.basicUsername}:${
            this.config.basicPassword
          }@${this.config.repoUrl}`;
        }
    ...
    WIKI.logger.info(
      `(STORAGE/GIT) Checking out branch ${this.config.branch}...`
    );
   ...
   */
  async sync() {
    const currentCommitLog = _.get(
      await this.git.log(["-n", "1", this.config.branch]),
      "latest",
      {}
    );
    ...
    if (_.includes(["sync", "push"], this.mode)) {
      WIKI.logger.info(
        `(STORAGE/GIT) Performing push to origin on branch ${
          this.config.branch
        }...`
      );
    ...
    if (_.includes(["sync", "pull"], this.mode)) {
      ...
      const diff = await this.git.diffSummary([
        "-M",
        currentCommitLog.hash,
        latestCommitLog.hash
      ]);
  ...
  async processFiles(files) {
    for (const item of files) {
      ...
      let itemContents = "";
      try {
        itemContents = await fs.readFile(
          path.join(this.repoPath, item.file),
          "utf8"
        );
        const pageData = WIKI.models.pages.parseMetadata(
          itemContents,
          contentType
        );
        ...
        if (currentPage) {
          ...
          WIKI.logger.info(
            `(STORAGE/GIT) Page marked as modified: ${item.file}`
          );
          ...
          const pageEditor = await WIKI.models.editors.getDefaultEditor(
            contentType
          );
        ...
        if (
          ...
          WIKI.logger.info(
            `(STORAGE/GIT) Page marked as deleted: ${item.file}`
          );
  ...
  async renamed(page) {
    WIKI.logger.info(
      `(STORAGE/GIT) Committing file move from ${page.sourcePath} to ${
        page.destinationPath
      }...`
    );
    const sourceFilePath = `${page.sourcePath}.${getFileExtension(
      page.contentType
    )}`;
    const destinationFilePath = `${page.destinationPath}.${getFileExtension(
      page.contentType
    )}`;

    await this.git.mv(`./${sourceFilePath}`, `./${destinationFilePath}`);
    await this.git.commit(
      `docs: rename ${page.sourcePath} to ${destinationFilePath}`,
      destinationFilePath,
      {
        "--author": `"${page.authorName} <${page.authorEmail}>"`
      }
    ...
    await pipeline(
      ...
      new stream.Transform({
        ...
        transform: async (page, enc, cb) => {
          ...
          await fs.outputFile(
            filePath,
            pageHelper.injectPageMetadata(page),
            "utf8"
          );
          await this.git.add(`./${fileName}`);
          cb();
