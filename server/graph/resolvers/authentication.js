...
module.exports = {
  Query: {
    ...
    async activeStrategies(obj, args, context, info) {
      ...
      strategies = strategies.map((stg) => {
        const strategyInfo =
          _.find(WIKI.data.authentication, ['key', stg.strategyKey]) || {}
        return {
          ...stg,
  ...
  AuthenticationMutation: {
    ...
    async loginChangePassword(obj, args, context) {
      try {
        const authResult = await WIKI.models.users.loginChangePassword(
          args,
          context
        )
        return {
    ...
    async updateStrategies(obj, args, context) {
      try {
        const previousStrategies =
          await WIKI.models.authentication.getStrategies()
        for (const str of args.strategies) {
          ...
          if (_.some(previousStrategies, ['key', str.key])) {
            await WIKI.models.authentication
              .query()
              .patch({
                key: str.key,
        ...
        for (const str of _.differenceBy(
          previousStrategies,
          args.strategies,
          'key'
        )) {
          const hasUsers = await WIKI.models.users
            .query()
            .count('* as total')
            .where({ providerKey: str.key })
            .first()
          if (_.toSafeInteger(hasUsers.total) > 0) {
            throw new Error(
              `Cannot delete ${str.displayName} as 1 or more users are still using it.`
            )
          } else {
            await WIKI.models.authentication
              .query()
    ...
    async resetGuestUser(obj, args, context) {
      try {
        ...
        return {
          responseResult: graphHelper.generateSuccess(
            'Guest user has been reset successfully.'
          )
        }
      } catch (err) {
        return graphHelper.generateError(err)
  ...
  AuthenticationStrategy: {
    icon(ap, args) {
      return fs
        .readFile(
          path.join(WIKI.ROOTPATH, `assets/svg/auth-icon-${ap.key}.svg`),
          'utf8'
        )
        .catch((err) => {
          if (err.code === 'ENOENT') {
