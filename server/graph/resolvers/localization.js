...
module.exports = {
  Query: {
    async locales(obj, args, context, info) {
      ...
      let localLocales = await WIKI.models.locales
        .query()
        .select(
          'code',
          'isRTL',
      ...
      return _.map(remoteLocales, (rl) => {
        ...
        return {
          ...rl,
          isInstalled,
          installDate: isInstalled ?
            _.find(localLocales, ['code', rl.code]).updatedAt :
            null
        }
      })
    },
    async localizationConfig(obj, args, context, info) {
      return {
  ...
  LocalizationMutation: {
    ...
    async updateLocale(obj, args, context) {
      try {
        ...
        const newLocale = await WIKI.models.locales
          .query()
          .select('isRTL')
          .where('code', args.locale)
          .first()
