...
const PG_ADVISORY_LOCK_KEY = 8400315
...
module.exports = async (pageId) => {
  ...
  try {
    ...
    const usePgLock = WIKI.config.db.type === 'postgres'
    if (usePgLock) {
      await WIKI.models.knex.raw('SELECT pg_advisory_lock(?)', [PG_ADVISORY_LOCK_KEY])
    }
    ...
    try {
      ...
      if (usePgLock) {
        await WIKI.models.knex.raw('SELECT pg_advisory_unlock(?)', [PG_ADVISORY_LOCK_KEY])
      }
    }
