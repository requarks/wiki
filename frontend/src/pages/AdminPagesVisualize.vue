  ...
  methods: {
    ...
    hierarchy(pages) {
      ...
      function recurse(depth, [parent, descendants]) {
        ...
        const descendantsByChild = Object.entries(
          _.groupBy(descendants, (page) => truncatePath(page.path))
        )
          .map(([childPath, descendantsGroup]) => [getPage(childPath), descendantsGroup])
          .map(([child, descendantsGroup]) => [
            child,
            _.filter(descendantsGroup, (d) => d.path !== child.path)
          ])
        return {
          ...parent,
    ...
    drawRelations() {
      ...
      const tree = d3.cluster().size([2 * Math.PI, this.radius - 100])
      ...
      const root = tree(
        this.bilink(
          d3
            .hierarchy(data)
            .sort(
              (a, b) => d3.ascending(a.height, b.height) || d3.ascending(a.data.path, b.data.path)
            )
        )
      )
    ...
    drawTree() {
      ...
      const svg = d3.create('svg').attr('viewBox', [0, 0, this.width, x1 - x0 + root.dx * 2])
    ...
    drawRadialTree() {
      ...
      const root = tree(d3.hierarchy(data).sort((a, b) => d3.ascending(a.data.title, b.data.title)))

      const svg = d3.create('svg').style('font', '10px sans-serif')
