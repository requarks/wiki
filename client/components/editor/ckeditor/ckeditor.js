    ...
    feeds: [
      {
        ...
        feed: async (queryText) => {
          // Check for partial matches in the cache
          const cachedResults = Object.keys(mentionCache)
          ...
          // If no partial matches, fetch from the backend
          let response
          try {
            const siteId = WIKI.$store.state.page.siteId
            response = await WIKI.$apollo.query({
          ...
          // Cache the results
          mentionCache[queryText] = mentions
          return mentions
        },
        minimumCharacters: 1
      }
